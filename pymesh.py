...
class Mesh:
    ...
    def enable_connectivity(self) -> None:
        ...
        if smart_rust is not None:
            self._adjacency = smart_rust.tetra_adjacency(self.voxels.tolist())
            return
        ...
        if name == "voxel_volume":
            if smart_rust is not None:
                self._attributes[name] = np.asarray(
                    smart_rust.tetra_volumes(self.vertices.tolist(), self.voxels.tolist()),
                    dtype=float,
                )
            else:
            ...
            if smart_rust is not None:
                self._attributes[name] = np.asarray(
                    smart_rust.tetra_centroids(self.vertices.tolist(), self.voxels.tolist()),
                    dtype=float,
                )
            else:
...
def _load_gmsh(path: Path) -> Mesh:
    if smart_rust is not None:
        vertices, faces, voxels = smart_rust.load_gmsh(str(path))
...
def _surface_faces(voxels: np.ndarray) -> np.ndarray:
    if smart_rust is not None:
        return np.array(smart_rust.tetra_surface_faces(voxels.tolist()), dtype=int).reshape((-1, 3))
