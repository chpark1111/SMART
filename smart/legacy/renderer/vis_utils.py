...
class Tree(object):
    ...
    class Node(object):
        def __init__(
            ...
            )  # all of its children relationships; each entry is a tuple <part_a, part_b, type, params, dist>
            ...
            """
            self.hyperedges = (
                [] if hyperedges is None else hyperedges
            )  # hyperedges are an alternative to edges (multiple children are connected by hyperedges)
            """
                Hyperedge format:
        ...
        def _to_str(self, level, pid, detailed=False):
            ...
            if detailed:
                out_str += (
                    "Box(" + ";".join([str(item) for item in self.box.numpy()]) + ")\n"
                )
            else:
                out_str += "\n"
        ...
        def print_tree(self):
            ...
            while len(stack) > 0:
                ...
                node_label = (
                    node.label
                    if (node.label is not None and len(node.label) > 0)
                    else "<no label>"
                )

                prefix = "".join([" |"] * (node_d - 1) + [" ��"] * (node_d > 0))
                s = s + "{}{}:{}\n".format(
                    prefix, Tree.NodeType(node.type.item()).name, node_label
                )
            ...
            for node in nodes:
                ...
                for edge in node.edges:
                    if leafs_only and not (
                        node.children[edge["part_a"]].is_leaf
                        and node.children[edge["part_b"]].is_leaf
                    ):
                        continue
                ...
                if type == "label_xzy":
                    ...
                    indices.sort(
                        key=lambda i: (
                            ...
                            node.children[i].box[0] * 100
                            + node.children[i].box[2] * 10
                            + node.children[i].box[1] * 1
                            if node.children[i].box is not None
                            else 0,
                        )
                    )
                    ...
                    for hyperedge in node.hyperedges:
                        hyperedge["parts"] = [
                            indices.index(p) for p in hyperedge["parts"]
                        ]
                else:
...
def load_object(fn, load_geo=False, load_geo_feat=False):
    ...
    while len(stack) > 0:
        ...
        if load_geo:
            if node_json["id"] >= 0:
                node.geo = (
                    geo_data["parts"][node_json["id"]].astype("float32").reshape(1, -1, 3)
                )
            else:
                parent_id = parent.part_id
                other_idx = np.where(geo_data["other_ids"] == parent_id)[0][0]
                node.geo = (
                    geo_data["other_parts"][other_idx].astype("float32").reshape(1, -1, 3)
                )

        if load_geo_feat:
            if node_json["id"] >= 0:
                node.geo_feat = (
                    geo_data["parts"][node_json["id"]].astype("float32").reshape(1, -1)
                )
            else:
                parent_id = parent.part_id
                other_idx = np.where(geo_data["other_ids"] == parent_id)[0][0]
                node.geo_feat = (
                    geo_data["other_parts"][other_idx].astype("float32").reshape(1, -1)
                )
        ...
        if "children" in node_json:
            for ci, child in enumerate(node_json["children"]):
                stack.append(
                    StackElement(
                        node_json=node_json["children"][ci],
                        parent=node,
                        parent_child_idx=ci,
                    )
                )
        ...
        if parent is None:
            ...
            if len(parent.children) <= parent_child_idx:
                parent.children.extend(
                    [None] * (parent_child_idx + 1 - len(parent.children))
                )
