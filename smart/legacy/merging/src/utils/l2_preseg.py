...
try:
    from chainer import cuda  # type: ignore
except ModuleNotFoundError:
    class _NumpyCudaFallback:
        @staticmethod
        def get_array_module(*args):
            return np

    cuda = _NumpyCudaFallback()
...
def l2_norm(x, y):
    ...
    """
    return ((x - y) ** 2).sum(axis=x.ndim - 1)
...
def farthest_point_sampling(
    ...
    if initial_idx is None:
        indices[:, 0] = xp.random.randint(len(pts[0]))
    else:
...
def distance_based_partition(pts, seed_idx):
    ret = []
    idx = np.array([i for i in range(len(pts))])

    centroids = pts[seed_idx]
    assigned_centroids = get_closest_centroid(pts[:, None, :], centroids[None, :, :])

    for c in range(centroids.shape[0]):
        cluster_members = idx[assigned_centroids == c]
        ret.append(cluster_members)
