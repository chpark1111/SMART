...
def presegmentation(data_path, tetmsh, preseg_type, path_to_bbox, fn, debug=False) -> List[list]:
    ...
    elif preseg_type == "bbox":
        result_path = os.path.join(os.path.join(path_to_bbox, fn), "result")
    ...
    elif preseg_type == "coacd":
        mesh_fold = os.path.join(data_path, "coacd")
        mshes = os.listdir(mesh_fold)

        for msh in mshes:
            mesh = trimesh.exchange.load.load(
                os.path.join(mesh_fold, msh), file_type="obj", process=False
            )
            trimesh.repair.fix_normals(mesh)
            mesh_list.append(mesh)
...
if __name__ == "__main__":
    dataset = "shapenet_airplane_e0.002_l0.1"  # "tetmesh_table_0.01_0.05"
    data = "1e0a24e1135e75a831518807a840c4f4"  # "4b455e9b8dc1380dbd508cb7996d6164"
    data_path = "../Mesh2Tet/final_data/%s/%s/" % (dataset, data)

    msh_file = os.path.join(data_path, "tetra.msh")
    tetmsh = pymesh.load_mesh(msh_file)

    presegmentation(data_path, tetmsh, coacd=True, debug=True)
