...
class ReplayBuffer:
    def __init__(
        ...
        self.bbox_obs_mem = np.zeros(
            (self.capacity, max_num_bbox, bbox_idim), dtype=np.float32
        )
        self.step_vec_mem = np.zeros((self.capacity, max_step), dtype=np.float32)
        self.action_mask_mem = np.zeros((self.capacity, max_num_action), dtype=np.float32)
        ...
        self.next_bbox_obs_mem = np.zeros(
            (self.capacity, max_num_bbox, bbox_idim), dtype=np.float32
        )
        self.next_step_vec_mem = np.zeros((self.capacity, max_step), dtype=np.float32)
        self.next_action_mask_mem = np.zeros(
            (self.capacity, max_num_action), dtype=np.float32
        )

        self.action_mem = np.zeros((self.capacity, 1), dtype=np.int32)
    ...
    def store(
        ...
        self.tet_obs_mem[index, :] = last_obs[0]
        self.bbox_obs_mem[index, :] = last_obs[1]
        self.step_vec_mem[index, :] = last_obs[2]
        self.action_mask_mem[index, :] = last_obs[3]

        self.reward_mem[index, :] = reward

        self.next_tet_obs_mem[index, :] = new_obs[0]
        self.next_bbox_obs_mem[index, :] = new_obs[1]
        self.next_step_vec_mem[index, :] = last_obs[2]
        self.next_action_mask_mem[index, :] = new_obs[3]
    ...
    def to_torch(
        self,
        array: Union[np.ndarray, Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]],
        copy: bool = True,
    ) -> Union[
