...
# Keep this module as a native compatibility facade for older internal scripts.
# It resolves to smart._cpp when available and otherwise uses Python fallbacks.
_backend = None
...
if os.environ.get("SMART_DISABLE_CPP", "").lower() in {"1", "true", "yes", "on"}:
    ...
    try:
        from . import _cpp as _cpp_backend  # type: ignore
    except ImportError:
        try:
            import _cpp as _cpp_backend  # type: ignore
        except ImportError:
            _cpp_backend = None

# Feature names preserved for older compatibility tests and scripts.  The
# implementation now resolves to smart._cpp first and then pure Python fallbacks.
NATIVE_COMPAT_FEATURES = (
    "BBoxState",
    "TetClippingState",
    "ManifoldBridgeMesh",
    "ManifoldState",
    ...
    "native_bbox_volumes",
    "coverage_mask",
    "native_coverage_mask",
    "centroid_proxy_axis_rewards",
    "native_centroid_proxy_axis_rewards",
    "action_count",
    "action_scales",
    "action_indices",
    "native_action_indices",
    "opposite_action",
    "opposite_actions",
    ...
    "native_action_count",
    "native_action_scales",
    "native_opposite_actions",
    "native_child_action_mask",
    "apply_axis_action",
    "native_apply_axis_action",
    "action_upper_rewards",
    "native_action_upper_rewards",
    "bbox_action_upper_rewards",
    "native_bbox_action_upper_rewards",
    "bbox_valid_mask",
    "native_bbox_valid_mask",
    "total_bbox_volume",
    "native_total_bbox_volume",
    "native_normalize_vertices_raw",
    "bbox_union_bounds",
    "native_bbox_union_bounds",
    "bbox_union_volume",
    "native_bbox_union_volume",
    "native_box_mesh",
    "bbox_rot_state_key",
    "bavf_scores",
    "native_bavf_scores",
    "merge_bavf_reward",
    "native_merge_bavf_reward",
    "softmax_scaled",
    "native_softmax_scaled",
    "ucb_scores",
    "ucb_best_indices",
    "incremental_average",
    "native_incremental_average",
    "discounted_reward",
    "native_discounted_reward",
    "native_best_ucb_child",
    "native_ucb_best_count",
    "native_prob_skip_exploration",
    "symmetric_chamfer",
    "native_symmetric_chamfer",
    "tetra_volumes",
    "native_tetra_volumes",
    "tetra_centroids",
    "native_tetra_centroids",
    "tetra_surface_faces",
    "native_tetra_surface_faces",
    "tetra_adjacency",
    "native_tetra_adjacency",
    "native_load_gmsh",
    "load_gmsh",
    "native_save_gmsh",
    "save_gmsh",
    "partition_summaries",
    "native_partition_summaries",
...
_CPP_COMPAT_ALIASES = {
    "bbox_volumes": "native_bbox_volumes",
    "coverage_mask": "native_coverage_mask",
    "centroid_proxy_axis_rewards": "native_centroid_proxy_axis_rewards",
    "action_count": "native_action_count",
...
def backend_path() -> str | None:
    if _cpp_backend is not None:
        return getattr(_cpp_backend, "__file__", None)
    return None


def native_core_available() -> bool:
    if _has_cpp_function("native_core_available"):
        return bool(_cpp_backend.native_core_available())
    if _has_backend_function("native_core_available"):
...
def backend_features() -> dict[str, bool]:
    return {
        name: _has_backend_bbox_state() if name == "BBoxState" else _has_native_feature(name)
        for name in NATIVE_COMPAT_FEATURES
    }
...
def _has_cpp_function(name: str) -> bool:
    return _cpp_backend is not None and hasattr(_cpp_backend, name)


def _has_native_feature(name: str) -> bool:
    alias = _CPP_COMPAT_ALIASES.get(name)
    if alias is not None and _has_cpp_function(alias):
        return True
    return _has_cpp_function(name) or _has_backend_function(name)


def _has_backend_bbox_state() -> bool:
    if _has_cpp_function("BBoxState"):
        bbox_state = _cpp_backend.BBoxState
    elif _has_backend_function("BBoxState"):
...
if _has_backend_bbox_state():
    BBoxState = _cpp_backend.BBoxState if _has_cpp_function("BBoxState") else _backend.BBoxState  # type: ignore
else:
...
TetClippingState = (  # type: ignore
    _cpp_backend.TetClippingState
    if _has_cpp_function("TetClippingState")
    else (_backend.TetClippingState if _has_backend_function("TetClippingState") else None)
)
ManifoldBridgeMesh = (  # type: ignore
    _cpp_backend.ManifoldBridgeMesh
    if _has_cpp_function("ManifoldBridgeMesh")
    else (_backend.ManifoldBridgeMesh if _has_backend_function("ManifoldBridgeMesh") else None)
)
ManifoldState = (  # type: ignore
    _cpp_backend.ManifoldState
    if _has_cpp_function("ManifoldState")
    else (_backend.ManifoldState if _has_backend_function("ManifoldState") else None)
)
CandidateBitsetState = (  # type: ignore
    _cpp_backend.CandidateBitsetState
    if _has_cpp_function("CandidateBitsetState")
    else (_backend.CandidateBitsetState if _has_backend_function("CandidateBitsetState") else None)
)


def manifold_bridge_available() -> bool:
    if _has_cpp_function("manifold_bridge_available"):
        return bool(_cpp_backend.manifold_bridge_available())
...
def manifold_cube_volume(x: float, y: float, z: float) -> float:
    if _has_cpp_function("manifold_cube_volume"):
        return float(_cpp_backend.manifold_cube_volume(float(x), float(y), float(z)))
...
def manifold_axis_box_intersection_volume(
    ...
    if _has_cpp_function("manifold_axis_box_intersection_volume"):
        verts = [[float(value) for value in row] for row in vertices]
        face_rows = [[int(value) for value in row] for row in faces]
        box = [float(value) for value in bounds]
        return float(_cpp_backend.manifold_axis_box_intersection_volume(verts, face_rows, box))
...
def bbox_volumes(bounds: Iterable[Iterable[float]]) -> list[float]:
    ...
    if _has_cpp_function("native_bbox_volumes"):
        return list(_cpp_backend.native_bbox_volumes(data))
    if _has_backend_function("native_bbox_volumes"):
        return list(_backend.native_bbox_volumes(data))
...
def native_bbox_volumes(bounds: Iterable[Iterable[float]]) -> list[float]:
    ...
    if _has_backend_function("native_bbox_volumes"):
        return list(_backend.native_bbox_volumes(data))
    return bbox_volumes(data)


def coverage_mask(points: Iterable[Iterable[float]], bounds: Iterable[float]) -> list[bool]:
    pts = [list(row) for row in points]
    box = list(bounds)
    if _has_cpp_function("native_coverage_mask"):
        return list(_cpp_backend.native_coverage_mask(pts, box))
    if _has_backend_function("native_coverage_mask"):
        return list(_backend.native_coverage_mask(pts, box))
...
def centroid_proxy_axis_rewards(
    ...
    if _has_cpp_function("native_centroid_proxy_axis_rewards"):
        return [
            (int(action), float(reward))
            for action, reward in _cpp_backend.native_centroid_proxy_axis_rewards(
                pts,
                vols,
                box_rows,
                rot_rows,
    ...
    if _has_backend_function("native_centroid_proxy_axis_rewards"):
        return [
            (int(action), float(reward))
            for action, reward in _backend.native_centroid_proxy_axis_rewards(
                pts,
                vols,
                box_rows,
                rot_rows,
...
def native_centroid_proxy_axis_rewards(
    ...
    return centroid_proxy_axis_rewards(
        pts,
        vols,
        box_rows,
        rot_rows,
...
def action_count(num_bbox: int, num_action_scale: int) -> int:
    if _has_cpp_function("native_action_count"):
        return int(_cpp_backend.native_action_count(num_bbox, num_action_scale))
    if _has_backend_function("native_action_count"):
        return int(_backend.native_action_count(num_bbox, num_action_scale))
...
def action_scales(num_action_scale: int) -> list[float]:
    if _has_cpp_function("native_action_scales"):
        return list(_cpp_backend.native_action_scales(num_action_scale))
    if _has_backend_function("native_action_scales"):
        return list(_backend.native_action_scales(num_action_scale))
...
def action_indices(num_bbox: int, num_action_scale: int) -> list[list[int]]:
    if _has_cpp_function("native_action_indices"):
        return [list(row) for row in _cpp_backend.native_action_indices(num_bbox, num_action_scale)]
    if _has_backend_function("native_action_indices"):
        return [list(row) for row in _backend.native_action_indices(num_bbox, num_action_scale)]
...
def opposite_action(action: int, num_action_scale: int) -> int:
    if _has_cpp_function("opposite_action"):
        return int(_cpp_backend.opposite_action(action, num_action_scale))
...
def opposite_actions(num_bbox: int, num_action_scale: int) -> list[int]:
    if _has_cpp_function("native_opposite_actions"):
        return list(_cpp_backend.native_opposite_actions(num_bbox, num_action_scale))
    if _has_backend_function("native_opposite_actions"):
        return list(_backend.native_opposite_actions(num_bbox, num_action_scale))
...
def opposite_action_mask(action: int, num_bbox: int, num_action_scale: int) -> list[bool]:
    if _has_cpp_function("opposite_action_mask"):
        return list(_cpp_backend.opposite_action_mask(int(action), int(num_bbox), int(num_action_scale)))
...
def untried_actions(action_mask: Iterable[bool]) -> list[int]:
    ...
    if _has_cpp_function("untried_actions"):
        return list(_cpp_backend.untried_actions(data))
...
def single_untried_action_mask(total_actions: int, action: int) -> list[bool]:
    if _has_cpp_function("single_untried_action_mask"):
        return list(_cpp_backend.single_untried_action_mask(int(total_actions), int(action)))
...
def mcts_child_action_mask(
    ...
    if _has_cpp_function("native_child_action_mask"):
        return list(
            _cpp_backend.native_child_action_mask(
                total_actions,
                action,
                num_action_scale,
                parent,
    ...
    if _has_backend_function("native_child_action_mask"):
        return list(
            _backend.native_child_action_mask(
                total_actions,
                action,
                num_action_scale,
                parent,
...
def native_action_count(num_bbox: int, num_action_scale: int) -> int:
    if _has_cpp_function("native_action_count"):
        return int(_cpp_backend.native_action_count(int(num_bbox), int(num_action_scale)))
    if _has_backend_function("native_action_count"):
        return int(_backend.native_action_count(int(num_bbox), int(num_action_scale)))
    return action_count(num_bbox, num_action_scale)


def native_action_scales(num_action_scale: int) -> list[float]:
    if _has_cpp_function("native_action_scales"):
        return list(_cpp_backend.native_action_scales(int(num_action_scale)))
    if _has_backend_function("native_action_scales"):
        return list(_backend.native_action_scales(int(num_action_scale)))
    return action_scales(num_action_scale)


def native_action_indices(num_bbox: int, num_action_scale: int) -> list[list[int]]:
    if _has_cpp_function("native_action_indices"):
        return [list(row) for row in _cpp_backend.native_action_indices(int(num_bbox), int(num_action_scale))]
    if _has_backend_function("native_action_indices"):
        return [list(row) for row in _backend.native_action_indices(int(num_bbox), int(num_action_scale))]
    return action_indices(num_bbox, num_action_scale)


def native_opposite_actions(num_bbox: int, num_action_scale: int) -> list[int]:
    if _has_cpp_function("native_opposite_actions"):
        return list(_cpp_backend.native_opposite_actions(int(num_bbox), int(num_action_scale)))
    if _has_backend_function("native_opposite_actions"):
        return list(_backend.native_opposite_actions(int(num_bbox), int(num_action_scale)))
    return opposite_actions(num_bbox, num_action_scale)
...
def native_child_action_mask(
    ...
    return mcts_child_action_mask(total_actions, action, num_action_scale, parent)
...
def apply_axis_action(
    ...
    data = [list(row) for row in bounds]
    if _has_cpp_function("native_apply_axis_action"):
        return [
            list(row)
            for row in _cpp_backend.native_apply_axis_action(
                data,
                int(action),
                int(num_action_scale),
                float(action_unit),
    ...
    if _has_backend_function("native_apply_axis_action"):
        return [
            list(row)
            for row in _backend.native_apply_axis_action(
                data,
                action,
                num_action_scale,
                float(action_unit),
...
def native_apply_axis_action(
    ...
    return apply_axis_action(data, action, num_action_scale, action_unit)
...
def action_upper_rewards(
    ...
    data = [list(row) for row in bounds]
    if _has_cpp_function("native_action_upper_rewards"):
        return list(
            _cpp_backend.native_action_upper_rewards(
                data,
                int(num_action_scale),
                float(action_unit),
                float(volume_sum),
    ...
    if _has_backend_function("native_action_upper_rewards"):
        return list(
            _backend.native_action_upper_rewards(
                data,
                int(num_action_scale),
                float(action_unit),
                float(volume_sum),
...
def native_action_upper_rewards(
    ...
    return action_upper_rewards(data, num_action_scale, action_unit, volume_sum, last_bbox_score)
...
def bbox_action_upper_rewards(
    ...
    data = [list(row) for row in bounds]
    if _has_cpp_function("native_bbox_action_upper_rewards"):
        return list(
            _cpp_backend.native_bbox_action_upper_rewards(
                data,
                int(bbox_idx),
                int(num_action_scale),
                float(action_unit),
    ...
    if _has_backend_function("native_bbox_action_upper_rewards"):
        return list(
            _backend.native_bbox_action_upper_rewards(
                data,
                int(bbox_idx),
                int(num_action_scale),
                float(action_unit),
...
def bbox_valid_mask(bounds: Iterable[Iterable[float]]) -> list[bool]:
    data = [list(row) for row in bounds]
    if _has_cpp_function("native_bbox_valid_mask"):
        return list(_cpp_backend.native_bbox_valid_mask(data))
    if _has_backend_function("native_bbox_valid_mask"):
        return list(_backend.native_bbox_valid_mask(data))
...
def native_bbox_valid_mask(bounds: Iterable[Iterable[float]]) -> list[bool]:
    ...
    if _has_backend_function("native_bbox_valid_mask"):
        return list(_backend.native_bbox_valid_mask(data))
    return bbox_valid_mask(data)


def total_bbox_volume(bounds: Iterable[Iterable[float]]) -> float:
    data = [list(row) for row in bounds]
    if _has_cpp_function("native_total_bbox_volume"):
        return float(_cpp_backend.native_total_bbox_volume(data))
    if _has_backend_function("native_total_bbox_volume"):
        return float(_backend.native_total_bbox_volume(data))
...
def native_normalize_vertices(
    ...
    *,
    ...
    data = [list(row) for row in vertices]
    if _has_cpp_function("native_normalize_vertices_raw"):
        normalized, raw_stats = _cpp_backend.native_normalize_vertices_raw(
            data,
            str(mode),
            str(center),
            float(target),
        )
        stats = _native_normalization_stats(list(raw_stats))
        return [tuple(float(value) for value in row) for row in normalized], stats
    if not _has_backend_function("native_normalize_vertices_raw"):
        raise RuntimeError("native vertex normalization is not available")
    normalized, raw_stats = _backend.native_normalize_vertices_raw(
        data,
        str(mode),
        str(center),
        float(target),
...
def bbox_union_bounds(bounds: Iterable[Iterable[float]]) -> list[float]:
    data = [list(row) for row in bounds]
    if _has_cpp_function("native_bbox_union_bounds"):
        return list(_cpp_backend.native_bbox_union_bounds(data))
    if _has_backend_function("native_bbox_union_bounds"):
        return list(_backend.native_bbox_union_bounds(data))
...
def native_bbox_union_bounds(bounds: Iterable[Iterable[float]]) -> list[float]:
    ...
    if _has_backend_function("native_bbox_union_bounds"):
        return list(_backend.native_bbox_union_bounds(data))
    return bbox_union_bounds(data)


def bbox_union_volume(bounds: Iterable[Iterable[float]]) -> float:
    data = [list(row) for row in bounds]
    if _has_cpp_function("native_bbox_union_volume"):
        return float(_cpp_backend.native_bbox_union_volume(data))
    if _has_backend_function("native_bbox_union_volume"):
        return float(_backend.native_bbox_union_volume(data))
...
def native_box_mesh(
    ...
    rot = [float(value) for value in rotation]
    if _has_cpp_function("native_box_mesh"):
        vertices, faces = _cpp_backend.native_box_mesh(
            float(x), float(y), float(z), float(lx), float(ly), float(lz), rot
        )
    elif _has_backend_function("native_box_mesh"):
        vertices, faces = _backend.native_box_mesh(
            float(x), float(y), float(z), float(lx), float(ly), float(lz), rot
        )
    else:
        base = [float(x), float(y), float(z)]
        lengths = [float(lx), float(ly), float(lz)]
        vertices = []
        for i in range(2):
            for j in range(2):
                for k in range(2):
                    vertices.append(
                        [
                            base[0] + rot[0] * i * lengths[0] + rot[3] * j * lengths[1] + rot[6] * k * lengths[2],
                            base[1] + rot[1] * i * lengths[0] + rot[4] * j * lengths[1] + rot[7] * k * lengths[2],
                            base[2] + rot[2] * i * lengths[0] + rot[5] * j * lengths[1] + rot[8] * k * lengths[2],
                        ]
                    )
        faces = [
            [1, 3, 0],
...
def bbox_rot_state_key(
    ...
    if _has_cpp_function("bbox_rot_state_key"):
        return str(_cpp_backend.bbox_rot_state_key(boxes, rots))
...
def bavf_scores(part_volumes: Iterable[float], bbox_volumes_: Iterable[float], alpha: float = 100.0) -> list[float]:
    ...
    if _has_cpp_function("native_bavf_scores"):
        return list(_cpp_backend.native_bavf_scores(parts, boxes, float(alpha)))
    if _has_backend_function("native_bavf_scores"):
        return list(_backend.native_bavf_scores(parts, boxes, float(alpha)))
...
def native_bavf_scores(part_volumes: Iterable[float], bbox_volumes_: Iterable[float], alpha: float = 100.0) -> list[float]:
    parts = list(part_volumes)
    boxes = list(bbox_volumes_)
    if _has_cpp_function("native_bavf_scores"):
        return list(_cpp_backend.native_bavf_scores(parts, boxes, float(alpha)))
    if _has_backend_function("native_bavf_scores"):
        return list(_backend.native_bavf_scores(parts, boxes, float(alpha)))
    return bavf_scores(parts, boxes, alpha)
...
def merge_bavf_reward(
    ...
    if _has_cpp_function("native_merge_bavf_reward"):
        return float(
            _cpp_backend.native_merge_bavf_reward(
                float(prev_bvs),
    ...
    if _has_backend_function("native_merge_bavf_reward"):
        return float(
            _backend.native_merge_bavf_reward(
                float(prev_bvs),
                float(left_bbox_volume),
                float(right_bbox_volume),
                float(merged_bbox_volume),
...
def softmax_scaled(values: Iterable[float], scale: float = 100.0) -> list[float]:
    ...
    if _has_cpp_function("native_softmax_scaled"):
        return list(_cpp_backend.native_softmax_scaled(data, float(scale)))
    if _has_backend_function("native_softmax_scaled"):
        return list(_backend.native_softmax_scaled(data, float(scale)))
...
def native_softmax_scaled(values: Iterable[float], scale: float = 100.0) -> list[float]:
    ...
    if _has_backend_function("native_softmax_scaled"):
        return list(_backend.native_softmax_scaled(data, float(scale)))
    return softmax_scaled(data, scale)
...
def ucb_scores(
    ...
    if _has_cpp_function("ucb_scores"):
        return list(_cpp_backend.ucb_scores(int(parent_visits), qs, visits, float(exp_weight)))
...
def ucb_best_indices(
    ...
    qs = [float(value) for value in child_qs]
    visits = [int(value) for value in child_visits]
    if _has_cpp_function("ucb_best_indices"):
        return list(
            _cpp_backend.ucb_best_indices(
                int(parent_visits),
                qs,
                visits,
                float(exp_weight),
...
def incremental_average(previous: float, count: int, value: float) -> float:
    if _has_cpp_function("native_incremental_average"):
        return float(_cpp_backend.native_incremental_average(float(previous), int(count), float(value)))
    if _has_backend_function("native_incremental_average"):
        return float(_backend.native_incremental_average(float(previous), int(count), float(value)))
...
def native_incremental_average(previous: float, count: int, value: float) -> float:
    ...
    if _has_backend_function("native_incremental_average"):
        return float(_backend.native_incremental_average(float(previous), int(count), float(value)))
    return incremental_average(previous, count, value)


def discounted_reward(rewards: Iterable[float], gamma: float) -> float:
    data = [float(reward) for reward in rewards]
    if _has_cpp_function("native_discounted_reward") and len(data) >= 128:
        return float(_cpp_backend.native_discounted_reward(data, float(gamma)))
    if _has_backend_function("native_discounted_reward") and len(data) >= 128:
        return float(_backend.native_discounted_reward(data, float(gamma)))
...
def native_discounted_reward(rewards: Iterable[float], gamma: float) -> float:
    data = [float(reward) for reward in rewards]
    if _has_cpp_function("native_discounted_reward"):
        return float(_cpp_backend.native_discounted_reward(data, float(gamma)))
    if _has_backend_function("native_discounted_reward"):
        return float(_backend.native_discounted_reward(data, float(gamma)))
    return discounted_reward(data, gamma)
...
def native_best_ucb_child(
    ...
    qs = [float(value) for value in child_qs]
    visits = [int(value) for value in child_visits]
    if _has_cpp_function("native_best_ucb_child"):
        return int(
            _cpp_backend.native_best_ucb_child(
                int(parent_visits),
                qs,
                visits,
                float(exp_weight),
    ...
    if _has_backend_function("native_best_ucb_child"):
        return int(
            _backend.native_best_ucb_child(
                int(parent_visits),
                qs,
                visits,
                float(exp_weight),
    ...
    best = ucb_best_indices(parent_visits, qs, visits, exp_weight)
    if not best:
        raise ValueError("children must not be empty")
    return best[int(tie_pick) % len(best)]
...
def native_ucb_best_count(
    ...
    qs = [float(value) for value in child_qs]
    visits = [int(value) for value in child_visits]
    if _has_cpp_function("native_ucb_best_count"):
        return int(
            _cpp_backend.native_ucb_best_count(
                int(parent_visits),
                qs,
                visits,
                float(exp_weight),
    ...
    if _has_backend_function("native_ucb_best_count"):
        return int(
            _backend.native_ucb_best_count(
                int(parent_visits),
                qs,
                visits,
                float(exp_weight),
    ...
    return len(ucb_best_indices(parent_visits, qs, visits, exp_weight))
...
def native_prob_skip_exploration(
    ...
    rewards = [float(value) for value in child_rewards]
    qs = [float(value) for value in child_qs]
    if len(rewards) != len(qs):
        raise ValueError("child_rewards and child_qs must have the same length")
    if _has_cpp_function("native_prob_skip_exploration"):
        return float(
            _cpp_backend.native_prob_skip_exploration(
                float(parent_reward),
                rewards,
                qs,
                float(best_reward),
    ...
    if _has_backend_function("native_prob_skip_exploration"):
        return float(
            _backend.native_prob_skip_exploration(
                float(parent_reward),
                rewards,
                qs,
                float(best_reward),
    ...
    mx_q = 0.0
    for reward, q_value in zip(rewards, qs):
        if reward > float(parent_reward) and q_value > mx_q:
            mx_q = q_value
    return min(max(mx_q / (float(best_reward) + 1e-9), 0.0), float(skip_rate))
...
def symmetric_chamfer(left: Iterable[Iterable[float]], right: Iterable[Iterable[float]]) -> float:
    ...
    if _has_cpp_function("native_symmetric_chamfer"):
        return float(_cpp_backend.native_symmetric_chamfer(left_points, right_points))
    if _has_backend_function("native_symmetric_chamfer"):
        return float(_backend.native_symmetric_chamfer(left_points, right_points))
...
def native_symmetric_chamfer(left: Iterable[Iterable[float]], right: Iterable[Iterable[float]]) -> float:
    ...
    if _has_backend_function("native_symmetric_chamfer"):
        return float(_backend.native_symmetric_chamfer(left_points, right_points))
    return symmetric_chamfer(left_points, right_points)
...
def tetra_volumes(vertices: Iterable[Iterable[float]], voxels: Iterable[Iterable[int]]) -> list[float]:
    ...
    if _has_cpp_function("native_tetra_volumes"):
        return list(_cpp_backend.native_tetra_volumes(verts, tets))
    if _has_backend_function("native_tetra_volumes"):
        return list(_backend.native_tetra_volumes(verts, tets))
...
def native_tetra_volumes(vertices: Iterable[Iterable[float]], voxels: Iterable[Iterable[int]]) -> list[float]:
    ...
    if _has_backend_function("native_tetra_volumes"):
        return list(_backend.native_tetra_volumes(verts, tets))
    return tetra_volumes(verts, tets)


def tetra_centroids(vertices: Iterable[Iterable[float]], voxels: Iterable[Iterable[int]]) -> list[float]:
    verts = [[float(value) for value in row] for row in vertices]
    tets = [[int(value) for value in row] for row in voxels]
    if _has_cpp_function("native_tetra_centroids"):
        return list(_cpp_backend.native_tetra_centroids(verts, tets))
    if _has_backend_function("native_tetra_centroids"):
        return list(_backend.native_tetra_centroids(verts, tets))
...
def native_tetra_centroids(vertices: Iterable[Iterable[float]], voxels: Iterable[Iterable[int]]) -> list[float]:
    ...
    if _has_backend_function("native_tetra_centroids"):
        return list(_backend.native_tetra_centroids(verts, tets))
    return tetra_centroids(verts, tets)


def tetra_surface_faces(voxels: Iterable[Iterable[int]]) -> list[list[int]]:
    tets = [[int(value) for value in row] for row in voxels]
    if _has_cpp_function("native_tetra_surface_faces"):
        return [list(row) for row in _cpp_backend.native_tetra_surface_faces(tets)]
    if _has_backend_function("native_tetra_surface_faces"):
        return [list(row) for row in _backend.native_tetra_surface_faces(tets)]
...
def native_tetra_surface_faces(voxels: Iterable[Iterable[int]]) -> list[list[int]]:
    ...
    if _has_backend_function("native_tetra_surface_faces"):
        return [list(row) for row in _backend.native_tetra_surface_faces(tets)]
    return tetra_surface_faces(tets)


def tetra_adjacency(voxels: Iterable[Iterable[int]]) -> list[list[int]]:
    tets = [[int(value) for value in row] for row in voxels]
    if _has_cpp_function("native_tetra_adjacency"):
        return [list(row) for row in _cpp_backend.native_tetra_adjacency(tets)]
    if _has_backend_function("native_tetra_adjacency"):
        return [list(row) for row in _backend.native_tetra_adjacency(tets)]
...
def native_tetra_adjacency(voxels: Iterable[Iterable[int]]) -> list[list[int]]:
    ...
    if _has_backend_function("native_tetra_adjacency"):
        return [list(row) for row in _backend.native_tetra_adjacency(tets)]
    return tetra_adjacency(tets)


def load_gmsh(filename: str) -> tuple[list[list[float]], list[list[int]], list[list[int]]]:
    path = str(filename)
    if _has_cpp_function("native_load_gmsh"):
        vertices, faces, voxels = _cpp_backend.native_load_gmsh(path)
        return (
...
def native_load_gmsh(filename: str) -> tuple[list[list[float]], list[list[int]], list[list[int]]]:
    ...
    if _has_backend_function("native_load_gmsh"):
        vertices, faces, voxels = _backend.native_load_gmsh(path)
        return (
            [list(row) for row in vertices],
            [list(row) for row in faces],
            [list(row) for row in voxels],
        )
    return load_gmsh(path)
...
def save_gmsh(
    ...
    tets = [[int(value) for value in row] for row in voxels]
    if _has_cpp_function("native_save_gmsh"):
        _cpp_backend.native_save_gmsh(path, verts, tris, tets)
        return
...
def native_save_gmsh(
    ...
    if _has_cpp_function("native_save_gmsh"):
        _cpp_backend.native_save_gmsh(path, verts, tris, tets)
        return
    if _has_backend_function("native_save_gmsh"):
        _backend.native_save_gmsh(path, verts, tris, tets)
        return
    save_gmsh(path, verts, tris, tets)
...
def partition_summaries(
    ...
    if _has_cpp_function("native_partition_summaries"):
        part_volumes, part_bounds, part_points = _cpp_backend.native_partition_summaries(
    ...
    if _has_backend_function("native_partition_summaries"):
        part_volumes, part_bounds, part_points = _backend.native_partition_summaries(
            verts, tets, vols, parts, bool(unique_points)
        )
        return (
            list(part_volumes),
...
def native_partition_summaries(
    ...
    return partition_summaries(verts, tets, vols, parts, unique_points)
...
def tet_clipping_metrics(
    ...
    if _has_cpp_function("tet_clipping_metrics"):
        return {
            str(key): float(value)
            for key, value in _cpp_backend.tet_clipping_metrics(
                verts,
                tets,
                boxes,
    ...
    if not _has_backend_function("tet_clipping_metrics"):
        raise RuntimeError("Native tet_clipping_metrics kernel is not available")
    return {
        str(key): float(value)
...
def run_mcts_callbacks(
    ...
    if _has_cpp_function("run_mcts_callbacks"):
        return dict(
            _cpp_backend.run_mcts_callbacks(
                args,
                env,
                int(num_iter),
                action_prior_logits,
    ...
    if not _has_backend_function("run_mcts_callbacks"):
        raise RuntimeError("Native MCTS callback runner is not available")
...
def run_greedy_refine_callbacks(args, env) -> tuple[list[float], int]:
    if _has_cpp_function("run_greedy_refine_callbacks"):
        rewards, count = _cpp_backend.run_greedy_refine_callbacks(args, env)
        return list(rewards), int(count)
    if not _has_backend_function("run_greedy_refine_callbacks"):
        raise RuntimeError("Native greedy refine callback runner is not available")
