...
def test_rl_mlp_action_prior_trains_with_torch_and_scores_context(tmp_path) -> None:
    pytest.importorskip("torch")
    trace = tmp_path / "trace.jsonl"
    rows = [
        {
    ...
    assert payload["policy_type"] == "coord_scale_mlp_prior"
    assert payload["metadata"]["model_type"] == "offline_rl_mlp"
    assert payload["metadata"]["trainer_backend"] == "torch"
    assert payload["metadata"]["device"] == "cpu"
    assert payload["metadata"]["records_used"] == 3
    assert len(logits) == 3
    assert all(isinstance(value, float) for value in logits)


def test_policy_gradient_action_prior_scores_concrete_actions(tmp_path) -> None:
    pytest.importorskip("torch")
    trace = tmp_path / "trace.jsonl"
    rows = [
        {
    ...
    trace.write_text("\n".join(json.dumps(row) for row in rows) + "\n", encoding="utf-8")
    output = tmp_path / "pg_agent_prior.json"
    ...
    assert payload["metadata"]["model_type"] == "policy_gradient_agent"
    assert payload["metadata"]["trainer_backend"] == "torch"
    assert payload["metadata"]["records_used"] == 3
    assert payload["metadata"]["candidate_actions_seen"] == 78
    assert len(logits) == 3
    assert all(isinstance(value, float) for value in logits)
    assert len(set(round(value, 8) for value in logits)) > 1


def test_policy_gradient_action_prior_uses_candidate_records(tmp_path) -> None:
    pytest.importorskip("torch")
    ...
    trace.write_text("\n".join(json.dumps(row) for row in rows) + "\n", encoding="utf-8")
    output = tmp_path / "candidate_pg.json"
    ...
    assert payload["policy_type"] == "action_mlp_prior"
    assert payload["metadata"]["candidate_records_used"] == 2
    assert len(logits) == 2
    assert all(isinstance(value, float) for value in logits)
...
def test_cli_build_rl_mlp_prior(tmp_path, capsys) -> None:
    pytest.importorskip("torch")
    trace = tmp_path / "trace.jsonl"
    trace.write_text(
        "\n".join(
            [
                json.dumps({"category": "airplane", "coord_idx": 0, "scale_idx": 0, "reward": 1.0}),
                json.dumps({"category": "airplane", "coord_idx": 3, "scale_idx": 1, "reward": -1.0}),
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    printed = json.loads(capsys.readouterr().out)
    assert payload["policy_type"] == "coord_scale_mlp_prior"
    assert printed["model_type"] == "offline_rl_mlp"
    assert printed["trainer_backend"] == "torch"


def test_cli_build_policy_gradient_agent_prior(tmp_path, capsys) -> None:
    pytest.importorskip("torch")
    trace = tmp_path / "trace.jsonl"
    trace.write_text(
        "\n".join(
